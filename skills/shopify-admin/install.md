...
required env key(s) are set (SHOPIFY_STORE / SHOPIFY_STORE_CLIENT_ID / SHOPIFY_STORE_CLIENT_SECRET)
**in the environment of the process that runs the agent** — in split UI/brain projects the brain
has its own env file, and the keys must be there (keys set only on the UI silently no-op the
tools) — or tell the user to add them. State your install plan.

**Phase 2 — build it.** Fetch the playbook at
https://raw.githubusercontent.com/tfcbot/jarvis-skills/v0.2.0/skills/shopify-admin/main.md
